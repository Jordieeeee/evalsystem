...
export default function CurriculumBuilder() {
  ...
  // --- PREVIOUS SUBJECT MANAGEMENT STATES (RETAINED) ---
  ...
  // --- PREVIOUS ADD MANAGEMENT INTERACTION (RETAINED) ---
  const openAddModal = () => {
  ...
  // --- PREVIOUS EDIT MANAGEMENT INTERACTION (RETAINED) ---
  const openEditModal = (subject) => {
    ...
    setFormData({
      ...
      corequisites: Array.isArray(subject.corequisites) ? subject.corequisites : [subject.corequisites || '-'], 
  ...
  // --- PREVIOUS FORM ACTIONS MUTATION LOGIC (RETAINED) ---
  ...
  // --- PREVIOUS ARCHIVE MANAGEMENT FEATURE (RETAINED) ---
            ...
            <table className="w-full text-left text-xs text-slate-700">
              <thead>
                <tr className="text-[10px] text-slate-400/90 font-bold uppercase tracking-wider border-b border-slate-100 pb-3">
                  <th className="pb-3 w-[12%]">Code</th>
                  <th className="pb-3 w-[25%]">Title</th>
                  <th className="pb-3 text-center w-[6%]">Units</th>
                  <th className="pb-3 w-[15%]">Prerequisites</th>
                  <th className="pb-3 w-[15%]">Co-requisites</th>
                  {/* Dynamic Header Column based on Active Layout Context */}
                  <th className="pb-3 w-[15%]">
                    {isOldCurriculum ? "New Equivalents" : "Old Equivalencies"}
                  </th>
                  {!isOldCurriculum && <th className="pb-3 text-center w-[8%]">Type</th>}
                  <th className="pb-3 text-right w-[8%]">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100/70 font-semibold">
                {items.map((sub) => {
                  const prereqsArray = Array.isArray(sub.prerequisites) ? sub.prerequisites : (sub.prerequisites ? [sub.prerequisites] : []);
                  const coreqsArray = Array.isArray(sub.corequisites) ? sub.corequisites : (sub.corequisites ? [sub.corequisites] : []);
                  
                  const cleanedPrereqs = prereqsArray.filter(p => p && p !== '-');
                  const cleanedCoreqs = coreqsArray.filter(c => c && c !== '-');

                  // --- BIDIRECTIONAL MAPPING SELECTION LOGIC ---
                  let matchedEquivalencies = [];
                  const subjectCodeFormatted = sub.courseCode.replace(" ", "_").toUpperCase();

                  if (isOldCurriculum) {
                    // Match where the old course ID matches the old subject
                    matchedEquivalencies = curriculumMappings.filter(
                      m => m.id.toUpperCase() === subjectCodeFormatted
                    );
                  } else {
                    // Match where the mapping's newCourseCode field matches the new subject
                    matchedEquivalencies = curriculumMappings.filter(
                      m => m.newCourseCode.toUpperCase() === subjectCodeFormatted
                    );
                  }
                  ...
                  return (
                    <tr key={sub.id || sub.courseCode} className="hover:bg-slate-50/40 transition-colors">
                        ...
                        {cleanedCoreqs.length > 0 ? (
                          <div className="flex flex-wrap gap-1.5">
                            {cleanedCoreqs.map((coreq, index) => (
                              <span key={index} className="bg-indigo-50 text-indigo-700 text-[10px] font-bold px-2 py-0.5 rounded border border-indigo-100 shadow-2xs">
                                {coreq}
                              </span>
                            ))}
                          </div>
                      ...
                      {/* Bidirectional Equivalencies Cell Output */}
                      <td className="py-4">
                        {matchedEquivalencies.length > 0 ? (
                          <div className="flex flex-col gap-1">
                            {matchedEquivalencies.map((equiv) => (
                              <div key={equiv.id} className="text-[11px] font-medium text-slate-600 bg-slate-50 border border-slate-200/60 p-1.5 rounded">
                                {isOldCurriculum ? (
                                  <>
                                    <span className="font-bold text-slate-800">
                                      {equiv.newCourseCode.replace("_", " ")}
                                    </span>
                                  ...
                                  <>
                                    ...
                                    <span className="block text-slate-400 text-[10px] truncate text-ellipsis overflow-hidden">
                                      {equiv.courseTitle}
                                    </span>
                                  </>
                                )}
                          ...
                          <span className="text-slate-400 font-medium text-[11px] italic">No Match Mapped</span>
                        )}
                      </td>
                      ...
                      {/* --- PREVIOUS TABLE ROW ACTIONS BLOCK (RETAINED FOR EDIT/ARCHIVE) --- */}
  ...
  return (
    <div className="min-h-screen bg-slate-50/40 text-slate-800 p-8 font-sans antialiased text-left">
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center md:justify-between border-b border-slate-200 pb-5 mb-6">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5 flex items-center gap-1.5 font-medium">
            ...
            College Course Management Strategy Console Matrix Map
          </p>
        </div>
        
        {/* Trigger Button for Modal Creation */}
      ...
      {/* --- PREVIOUS MANAGEMENT FULL DIALOG POPUP FORM PANEL (RETAINED) --- */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-950/20 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-xl border border-slate-200 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4 text-xs font-semibold text-slate-600">
              ...
              <div>
                <label className="block text-slate-500 mb-1">Co-requisites (Comma Separated)</label>
                <input 
                  type="text" 
                  value={formData.corequisites.join(', ')} 
                  onChange={(e) => setFormData({...formData, corequisites: e.target.value.split(',').map(v => v.trim())})}
