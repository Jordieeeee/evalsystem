...
export interface AssignedStanding {
  year_level: number;
  semester: string;
}
...
export interface EvaluationInput {
  studentId: string;
  assignedStanding: AssignedStanding;
  transcript: TranscriptRow[];
}

type GradeClass = "PASSED" | "INC" | "FAILED" | "INVALID";

/** A new-curriculum requirement, normalized to one internal shape. */
interface NewSubject {
  ...
  year_level: number;
  semester: string;
  /** Doc id, so every plan entry is traceable back to an input document. */
  docId: string;
}

/** An old->new equivalency, normalized to one internal shape. */
interface Mapping {
  old_code: string;
  new_code: string;
...
export interface CreditedCourse {
  ...
  new_title: string;
  new_units: number;
  /** Which documents produced this credit. */
  source: { mappingDocId: string; newSubjectDocId: string };
}
...
export interface IncompleteCourse {
  ...
  ay_taken: string;
  semester_taken: string;
  /** Populated only when the INC maps to a known new requirement. */
  wouldSatisfy: { new_code: string; new_title: string; new_units: number } | null;
}
...
export interface PlanCourse {
  ...
  year_level: number;
  semester: string;
  /** True when an unresolved INC would satisfy this requirement. */
  incBlocked: boolean;
  newSubjectDocId: string;
}
 ...
 */
export function classify(grade: unknown): GradeClass {
  if (grade === null || grade === undefined || grade === "") return "INVALID";
  if (typeof grade === "boolean") return "INVALID";

  const n = Number(grade);
  if (Number.isNaN(n) || !Number.isFinite(n)) return "INVALID";

  if (n >= BEST_PASSING_GRADE && n <= LOWEST_PASSING_GRADE) return "PASSED";
  if (n === INC_GRADE) return "INC";
  if (n === FAILING_GRADE) return "FAILED";
...
function normalizeYearLevel(raw: unknown): number | null {
  ...
  if (s.includes("fourth") || s.includes("4")) return 4;
  if (s.includes("fifth") || s.includes("5")) return 5;
 ...
 */
function normalizeSemester(raw: unknown): string | null {
  const s = String(raw ?? "").toLowerCase().trim();
  if (!s) return null;
  if (s.includes("1st") || s.includes("first")) return "1st Semester";
  if (s.includes("2nd") || s.includes("second")) return "2nd Semester";
  if (s.includes("summer") || s.includes("mid")) return "Midyear";
  return null;
}

/** Term key exactly as authored in config/unit_targets: `Y${year}-${semester}`. */
export const termKey = (yearLevel: number, semester: string): string =>
  `Y${yearLevel}-${semester}`;
...
const termRank = (yearLevel: number, semester: string): number =>
  yearLevel * 10 + (SEMESTER_ORDER[semester] ?? 0);
...
function readNewSubject(docId: string, d: Record<string, unknown>): NewSubject | string {
  ...
  if (!canonical(code)) return `new_subjects/${docId}: unreadable course code — excluded from requirements.`;
  ...
  if (year === null) return `new_subjects/${docId}: unreadable year level — excluded from requirements.`;
  if (semester === null) return `new_subjects/${docId}: unreadable semester — excluded from requirements.`;
...
function readMapping(docId: string, d: Record<string, unknown>): Mapping | string {
  const oldCode = d.old_code ?? d.oldCourseCode ?? docId;
  const newCode = d.new_code ?? d.newCourseCode;
  if (!canonical(oldCode)) return `curriculum_mappings/${docId}: unreadable old_code — mapping ignored.`;
  if (!canonical(newCode)) return `curriculum_mappings/${docId}: unreadable new_code — mapping ignored.`;
...
function validateInput(data: unknown): EvaluationInput {
  ...
  if (typeof d.studentId !== "string" || d.studentId.trim() === "") {
    throw new HttpsError("invalid-argument", "studentId is required and must be a non-empty string.");
  }

  const standing = d.assignedStanding as Record<string, unknown> | undefined;
  if (typeof standing !== "object" || standing === null) {
    throw new HttpsError("invalid-argument", "assignedStanding is required and must be an object.");
  }
  const year = Number(standing.year_level);
  if (!Number.isFinite(year) || year < 1) {
    throw new HttpsError("invalid-argument", "assignedStanding.year_level must be a positive number.");
  }
  const sem = normalizeSemester(standing.semester);
  if (sem === null) {
    throw new HttpsError(
      "invalid-argument",
      `assignedStanding.semester is unreadable. Expected "1st Semester", "2nd Semester" or "Midyear".`
    );
  }

  if (!Array.isArray(d.transcript)) {
    throw new HttpsError("invalid-argument", "transcript must be an array.");
  }

  const transcript: TranscriptRow[] = d.transcript.map((row, i) => {
    if (typeof row !== "object" || row === null) {
      throw new HttpsError("invalid-argument", `transcript[${i}] must be an object.`);
    }
    const r = row as Record<string, unknown>;
    if (typeof r.old_code !== "string" || r.old_code.trim() === "") {
      throw new HttpsError("invalid-argument", `transcript[${i}].old_code is required and must be a non-empty string.`);
    }
    ...
    return {
      ...
      grade: r.grade as number | string,
 ...
 */
function reconcile(subjects: NewSubject[], targets: Record<string, number>): string[] {
  const warnings: string[] = [];
  const catalogTotal = subjects.reduce((sum, s) => sum + s.new_units, 0);
  const targetTotal = Object.values(targets).reduce<number>(
  ...
  if (catalogTotal !== targetTotal) {
    warnings.push(
      `RECONCILIATION MISMATCH: config/unit_targets totals ${targetTotal} units across ${
        Object.keys(targets).length
      } terms, but new_subjects totals ${catalogTotal} units across ${subjects.length} courses ` +
...
interface RawData {
  mappingDocs: { id: string; data: Record<string, unknown> }[];
  subjectDocs: { id: string; data: Record<string, unknown> }[];
...
async function readAll(): Promise<RawData> {
  ...
  try {
    ...
    return {
      mappingDocs: mappingsSnap.docs.map((d) => ({ id: d.id, data: d.data() as Record<string, unknown> })),
      subjectDocs: subjectsSnap.docs.map((d) => ({ id: d.id, data: d.data() as Record<string, unknown> })),
      targets: targetsSnap.exists ? ((targetsSnap.data() ?? {}) as Record<string, number>) : {},
    };
  } catch (err) {
    ...
    throw new HttpsError("internal", "Could not read curriculum data. Please retry; if this persists, contact the registrar system administrator.");
  }
}
...
export function evaluate(input: EvaluationInput, raw: RawData): EvaluationOutput {
  const warnings: string[] = [];

  // --- Normalize the catalog. Rejected docs are surfaced, never silently zeroed.
  const newSubjects: NewSubject[] = [];
  for (const { id, data } of raw.subjectDocs) {
    const result = readNewSubject(id, data);
    if (typeof result === "string") warnings.push(result);
    else if (data.isArchived !== true) newSubjects.push(result); // archived courses are not requirements
  }
  ...
  for (const { id, data } of raw.mappingDocs) {
    ...
    if (typeof result === "string") warnings.push(result);
    else mappings.push(result);
  }

  if (Object.keys(raw.targets).length === 0) {
    warnings.push(
      "config/unit_targets is missing or empty. Per-term targets and variances are reported as null; the plan itself is unaffected."
    );
  }
  ...
  const mappingByOldCode = new Map<string, Mapping>();
  for (const m of mappings) {
    const key = canonical(m.old_code);
    if (mappingByOldCode.has(key)) {
      warnings.push(
        `curriculum_mappings: duplicate equivalency for old code ${m.old_code} ` +
  ...
  const subjectByNewCode = new Map<string, NewSubject>();
  ...
  // --- CLASSIFY + BUCKET -----------------------------------------------------
  const credited: CreditedCourse[] = [];
  const unmapped: UnmappedCourse[] = [];
  const incomplete: IncompleteCourse[] = [];
  const excluded: ExcludedCourse[] = [];

  // new_code -> the credit that satisfied it. A requirement can be satisfied once.
  const satisfiedByCredit = new Map<string, CreditedCourse>();
  // new_code -> an unresolved INC that would satisfy it once the grade completes.
  const incBlockedNewCodes = new Set<string>();
  ...
  for (const row of input.transcript) {
    ...
    if (cls === "INC") {
      ...
      if (target) incBlockedNewCodes.add(canonical(target.new_code));
      if (mapping && !target) {
        warnings.push(
          `INC course ${row.old_code} maps to ${mapping.new_code} (curriculum_mappings/${mapping.docId}), but that code is not in new_subjects. Cannot flag the affected term.`
        );
      }
      incomplete.push({
        ...base,
    ...
    if (mapping.new_units !== null && mapping.new_units !== target.new_units) {
      warnings.push(
        `curriculum_mappings/${mapping.docId} states ${mapping.new_units} units for ${target.new_code}, but new_subjects/${target.docId} states ${target.new_units}. Credited ${target.new_units} (catalog is source of truth).`
      );
    }

    const credit: CreditedCourse = {
      ...base,
      grade,
  ...
  if (backlog.length > 0) {
    ...
    warnings.push(
      `${backlog.length} uncredited requirement(s) totalling ${backlogUnits} units fall in terms BEFORE the assigned standing ` +
        `(${termKey(input.assignedStanding.year_level, input.assignedStanding.semester)}) and are therefore not scheduled in the plan: ` +
        `${backlog.map((s) => `${s.new_code} (${termKey(s.year_level, s.semester)}, ${s.new_units}u)`).join(", ")}. ` +
        `They remain counted in summary.unitsRemaining. Either the standing is too advanced or these need a back-subject decision.`
    );
  }

  // --- GROUP BY TERM ---------------------------------------------------------
  const byTerm = new Map<string, PlanCourse[]>();
  for (const s of plannable) {
    const key = termKey(s.year_level, s.semester);
    ...
    else byTerm.set(key, [course]);
  }

  const plan: PlanTerm[] = Array.from(byTerm.entries())
    .map(([term, courses]) => {
  ...
  for (const code of incBlockedNewCodes) {
    if (!flaggedNewCodes.has(code)) {
      ...
      warnings.push(
        `An unresolved INC would satisfy ${subject?.new_code ?? code}, but that requirement is not in the plan (already credited, or in a term before the assigned standing). No term is flagged for it.`
      );
    }
  }
...
export const curriculumShiftEvaluation = onCall(
  { region: "asia-southeast1", memory: "256MiB", timeoutSeconds: 60 },
  async (request: CallableRequest): Promise<EvaluationOutput> => {
    // This decides a real student's credited units — it is not a public endpoint.
    if (!request.auth) {
      throw new HttpsError("unauthenticated", "You must be signed in to run a curriculum shift evaluation.");
    }
